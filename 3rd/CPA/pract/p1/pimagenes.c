...
// int ti = omp_get_wtime();
// int tf = omp_get_wtime();
// int timeLasted = tf - ti;
           ...
           int n, int m)
{
  ...
  // This loop can not be parallelized, because the last memcpy 
  // makes a dependency between the current interation and the next iteration
  for (p = 0; p < pasos; p++) {
    // fourth try (same: 1.81)
    // #pragma omp parallel for private(tot,r,g,b,v,l,k,j)  
    for (i = 0; i < n; i++) {
      // thrid try (faster: 1.81s)
      // #pragma omp parallel for private(tot,r,g,b,v,l,k) 
      for (j = 0; j < m; j++) {
        r = 0;
        g = 0;
        b = 0;
        ...
        // second try (this is fast: 5,14)
        // #pragma omp parallel for reduction(+:tot,r,g,b) private(v,l) 
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
          // first try (this is slow)
