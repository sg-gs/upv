...
public class PPool extends JPanel implements ActionListener {   
    ...
    private void simulate(String s)  {
        ...
        if (bErr) { 
            luz.yellow(); 
            System.out.println("Pool "+TYPE+" has not fullfilled its requirements");
        }
        else {
            luz.green();
        }
    ...
    private JPanel getPeopleCounterHeader (int kids, int instructors, int KI, int capacity) {
    ...
    public PPool(int k, int i) {
        ...
        JPanel header = getPeopleCounterHeader(K, I, KI, CAP);
        header.add(working);
        ...
        enableTypeRadioButtons(true);
        p.setLayout(new GridLayout(0,1)); p.add(header); p.add(ptype);
    ...
    public static void main(String[] args) {
        ...
        final int kids = integer(args,0,7,5,20);
        final int instructors = integer(args,1,3,2,5);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                PPool ppool = new PPool(kids, instructors);
