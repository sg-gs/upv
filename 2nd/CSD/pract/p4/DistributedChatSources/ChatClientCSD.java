...
public class ChatClientCSD implements MessageListener {
   ...
   //
   // For us, connect means to locate it, register a new user into it and retrieve
   // its channel list.
   ...
   public String[] doConnect(String serverName, String nick) throws Exception {
      ...
      try {
         ...
         // 1.b Look for the object "ChatServer" in the name server, using the previous
         // reference.
         // Remember that the object name is stored in the variable "serverName" (input
         // parameter of the "doConnect" method)
         // Store the obtained remote reference in the variable "srv" (defined at the
         // beginning of ChatClientCSD class)
         ...
         throw new Exception(
               "rmiregistry not found at '" + conf.getNameServiceHost() + ":" + conf.getNameServicePort() + "'");
      ...
      // Once we've got the server, we create a local user object and register it into
      // the server

      /* ******************************************************/
      /*
       * ACTIVITY 2. Create a local object "ChatUser" and register it in the server.
       * //2.a Create the ChatUser object, indicating as parameters the nick of the
       * user and "this" as MessageListener.
       */
      myUser = new ChatUser(nick, this);

      /*
       * 2.b Connect the client to the ChatServer, using the method "connectUser" of
       * ChatServer class. // Launch an exception if there is any error.
       */
      try {
      ...
      /*
       * 2.c Obtain the list of channels, using the method "listChannels" of
       * ChatServer.
       */
      ...
      // Convert channel list to string list, since we don't want the UI to know about
      // invocable
   ...
   //
   // UI wants to send a private message to some user... lets do it creating a
   // IChatMessage
   //
