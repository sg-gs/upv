...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    // protected final void rehashing() {
        ...
        // ListaConPI<EntradaHash<C, V>>[] nuevoArray = 
            // new LEGListaConPI[nuevaCapacidad];
        
        // for (int i = 0; i < nuevaCapacidad; i++) {
            // // añade huecos a partir del final del array hasta alcanzar la nueva capacidad deseada
            // if (i < elArray.length) {
                // nuevoArray[i] = elArray[i];
            // } else {
                // nuevoArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
            // }
        // }
    ...
    @SuppressWarnings("unchecked")
    protected final void rehashing() {
        ListaConPI<EntradaHash<C, V>>[] oldArr = elArray;
        ...
        ListaConPI<EntradaHash<C, V>> cubeta;
    ...
    public final double desviacionTipica() {
        int nCubetas = elArray.length;
        
        double longitudMedia = 0.0;
        
        for (int i = 0; i < nCubetas; i++) {
            longitudMedia += elArray[i].talla();
        }
        
        longitudMedia /= nCubetas;
        
        double longitudIEsima = 0.0;
        double sumatorio = 0.0;
        
        for (int j = 0; j < nCubetas; j++) {
           sumatorio += Math.pow((elArray[j].talla() - longitudMedia), 2);
        }
        
        return Math.sqrt(sumatorio / nCubetas);
    }
    ...
    public final double costeMLocalizar() {
        ...
        for (int i = 0; i < elArray.length; i++) {
            // (si hay un sólo elemento, no hay colisión)
            coste += (elArray[i].talla() * (elArray[i].talla() + 1))/2;
        }
        
        return (double) (coste / talla);
    }
