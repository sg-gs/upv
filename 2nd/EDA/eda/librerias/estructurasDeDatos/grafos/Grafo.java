...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        ...
        if (numVertices() - 1 != ordenVisita) { return null; }
    ...
    public Arista[] kruskal() {  
        ...
        int N = numVertices();
        UFSet cc = new ForestUFSet(N); 
        
        while (cardinalEPrima < N - 1 && !aristasFactibles.esVacia()) {
            Arista a = aristasFactibles.eliminarMin();
            
            int ccV = cc.find(a.getOrigen()); // a.getOrigen() = v
            int ccW = cc.find(a.getDestino()); // a.getDestino() = w
            
            if (ccV != ccW) {
                cc.union(ccV, ccW);
                
                res[cardinalEPrima++] = a;
            }
        }
        
        return (cardinalEPrima == N - 1) ? res : null;
    }
}
