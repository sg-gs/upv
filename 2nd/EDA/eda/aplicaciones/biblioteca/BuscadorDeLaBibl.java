...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        ListaConPI<Termino> legomenas = new LEGListaConPI<Termino>();
        ListaConPI<Termino> terminos = index.claves();
        
        ListaConPI<BuscadorDeLaBibl.Posting> postingsActuales;
        
        terminos.inicio();
        
        Termino terminoActual;
        
        while (!terminos.esFin()) {
            terminoActual = terminos.recuperar();
            postingsActuales = index.recuperar(terminoActual);
            
            if (postingsActuales.talla() == 1) {
                legomenas.insertar(terminoActual);
            }
            
            terminos.siguiente();
        }
        
        return legomenas.talla() == 0 ? null : legomenas;
    }
}    